...
abstract: >
  ...
  We show that the minimal divisional load is achieved by binary division trees with progenitor cells incapable of self-renewal.
...
The full model, which considers symmetric and asymmetric cell divisions, as well as cell death, is described in the Appendices.
We further consider the possibility for stem cells to be non-dividing in the full model, as it has been suggested that quiescence is another mechanism of cancer defence in hierarchical tissues \citep{adams:CSC:2015}.
...
In the full model we additionally consider a compartment of non-dividing (quiescent) SCs, labelled $i=0$.
See \ref{appendix:fullModel} for details.

```{r 1-model, fig.pos = "ht", out.width = "\\textwidth", fig.cap = caption}
caption <- "Reduced model schematic.
...
B) In this reduced model cells can only symmetrically self-renew or symmetrically differentiate and there are no quiescent SCs.
...
knitr::include_graphics("figures/1-model_reduced.pdf")
```
...
To build intuition and guide analytical calculations, we primarily consider the reduced model (\figref{fig:1-model}B) in which cells only divide symmetrically: We ignore cell death and asymmetric cell division, as well as SC quiescence compared to the full model that is described in \ref{appendix:fullModel}.
This reduced model is parametrised by $\beta_i$, which is the division rate of cells in compartment $i$, and $\alpha_i$, which is their probability of self-renewal (correspondingly, $1-\alpha_i$ is the probability of differentiation) following a division event.
The reduced model is described by the set of ordinary differential equations
\begin{equation}
...
These models yield identical results for the mean number of divisions in the reduced and full models, as described in \ref{appendix:divLoad-Reduced} and \ref{appendix:divLoad-Full}, respectively.
...
\end{equation}
This equation is equivalent to the result of \citet{derenyi:NatComms:2017} for the reduced dynamics.
...
The Lagrangian functions for the full model can be constructed analogously.
...
\end{equation}
By rearranging \eqref{eq:alpha-lambda} we can now define the values of $\alpha_i$ which minimise the number of divisions per TDC during homeostasis and for a given number of compartments.
These values will depend explicitly on the number of compartment size $n$, hence we write this value as $\hat{\alpha}(n)$, which satisfies
\begin{equation}
...
\label{eq:alpha}
\end{equation}
We here want to differentiate between the subscript notation, which is just the index of the compartment, and the bracket notation, denoting that the optimised parameter value $\hat{\alpha}(n)$ depends on the number of compartments in the hierarchy.

From \eqref{eq:alpha}, we see that an increase in the number of compartments $n$ must be compensated by a decrease in the progenitor self-renewal probability $\hat{\alpha}(n)$ to maintain the homeostatic level of TDCs.
...
The values of $\\hat{\\alpha}(n)$, $\\hat{N}_n$, and $\\hat{D}_n$ along these solid and dashed lines are shown in panels D, E, and F, respectively.
...
\end{equation}
Note that the SC division rate $\beta_1$ is now decoupled from the SC population size $N_1^*$ for this time-dependent problem, and therefore we can no longer consider SC turnover $\beta_1 N_1^*$ as an independently-variable quantity as was done in \figref{fig:2-noTime-Simple}.
...
Again, we here want to clarify the difference between the subscript notation, which is just the index of the compartment, and the bracket notation, denoting that the optimised parameter values $\hat{\alpha}(n)$ and $\hat{\beta}_1(n)$ depend on the number of compartments in the hierarchy.
...
The values of $\\hat{\\alpha}(n)$, $\\hat{\\beta}_1(n)$, $\\hat{N}_n$, and $\\hat{D}_n(t)$ along these solid and dashed lines are shown in panels E-H, respectively.
...
plot_grid(
  ...
  ggplot(data.time, aes(x = n, y = N.1)) +
    ...
    scale_fill_viridis_c(limits = c(0,50),
                         guide = guide_colourbar(
                           title = expression("divisions per TDC" ~ (hat(italic(D))[italic(n)])),
                           title.position = "top",
                           barwidth = 10,
                           title.hjust = 0.5
                         )
  ...
  ggplot(slice.data, aes(x = n, y = Dn, group = type, colour = type)) +
    ...
    theme(legend.position = "none") +
    labs(x = expression("number of compartments" ~ (italic(n))),
         y = expression("divisions per TDC" ~ (hat(italic(D))[italic(n)]))),
...
For the full model (see \ref{appendix:fullModel} for parameter descriptions), including quiescence and asymmetric division, we can repeat the above analyses using the Lagrangian function
\begin{equation}
...
We have therefore immediately set the death rate $d_i=0$ for $1\le i<n$.
We have also used the condition that SC self-renewal and SC differentiation occur at the same rate, $a_1=b_1$, which is required for homeostasis.
...
By increasing the SC differentiation flux $(2b_1+c_1)\frac{\gamma}{\ell+\gamma}N^*_{\rm SC}$, we can reduce the number of compartments while maintaining the TDC population, hence reducing the divisional load (as described for the reduced model).
...
Intuitively it is not clear if the total number of divisions per TDC, or only the number of divisions in the SC compartment, is important for cancer initiation and incidence. Tomasetti and Vogelstein have shown that across tissues the dependence of cancer incidence on the number of SC divisions is sub-linear, such that an increase in SC divisions does not lead to a proportional increase in cancer risk \citep{tomasetti:Science:2015,tomasetti:Science:2017}.
Therefore, other factors are at play, which could be extrinsic (e.g. smoking), or related to tissue architecture.
...
# Description of the full model {#appendix:fullModel}
In addition to the population structure used in the reduced model, we consider a compartment of non-dividing (quiescent) SCs, labelled $i=0$.
This structure is highlighted in \figref{fig:6-modelFull}A.

```{r 6-modelFull, fig.pos = "ht", out.width = "\\textwidth", fig.cap = caption}
caption <- "Full model schematic.
A) We consider a hierarchical population of cells, with stem cells (SC) at the top of the hierarchy giving rise to progenitor cells and ultimately terminally-differentiated cells (TDC).
...
knitr::include_graphics("figures/1-model_full.pdf")
```

In the full model (\figref{fig:6-modelFull}B) cells in compartment $i$ divide to generate two daughter cells in the same compartment (symmetric self-renewal) with rate $a_i$.
...
We further set $d_i=0$, as well as denoting $a_i = \alpha_i \beta_i$ and $b_i = (1-\alpha_i)\beta_i$.
The division rate $\beta_n$ assumes the role of the death rate of the TDCs, where we set $\alpha_n = 0$.

For the full model (\figref{fig:6-modelFull}B), the number of cells per compartment are described by the following ordinary differential equations (ODE):
\begin{equation}
\begin{aligned}
